...
class DataManager:
    ...
    def dashboard_data_generator(self):
        ...
        print("\n📊 大屏数据生成器")
        print("="*60)
        print("为Grafana大屏提供测试数据，支持两种模式:")
        print("1. 直接操作模式: 直接在Sink端ADS表进行CRUD操作，快速测试大屏显示效果")
        print("2. 架构模式: 通过Fluss架构在Source端生成数据并传播到Sink端")
        print("="*60)
        
        while True:
            print("\n🎯 请选择大屏数据生成模式:")
            print("1. 📈 直接操作大屏数据 - 快速测试大屏显示效果")
            print("2. 🏗️  架构化数据生成 - 遵循Fluss数仓架构")
            print("3. 📊 查看当前数据状态")
            print("0. 返回主菜单")
            ...
            if choice == '1':
                self._direct_dashboard_operations()
            elif choice == '2':
                self._fluss_architecture_generator()
            elif choice == '3':
                self._show_dashboard_data_status()
            elif choice == '0':
                break
            else:
                print("❌ 无效选择，请重试")
    
    def _direct_dashboard_operations(self):
        """直接操作大屏Sink端ADS表 - 快速测试模式"""
        print("\n📈 直接操作大屏数据模式")
        print("="*50)
        print("此模式直接在Sink端ADS表进行CRUD操作，用于快速测试大屏数据丰富度")
        print("⚠️  注意: 此模式绕过Fluss架构，仅用于测试目的")
        print("="*50)
        
        while True:
            print("\n🛠️  选择操作类型:")
            print("1. 🚀 快速生成测试数据 - 生成基础大屏数据")
            print("2. 📊 生成丰富历史数据 - 生成多样化历史数据")
            print("3. ⚡ 实时数据模拟 - 持续更新实时数据")
            print("4. 🗑️  清空大屏数据 - 清空所有ADS表数据")
            print("5. 🔧 修复数据问题 - 修复主键冲突等问题")
            print("0. 返回上级菜单")
            
            choice = input("\n请选择操作: ").strip()
            
            if choice == '1':
                self._generate_basic_dashboard_data_fixed()
            elif choice == '2':
                self._generate_rich_dashboard_data_fixed()
            elif choice == '3':
                self._generate_realtime_dashboard_data_fixed()
            elif choice == '4':
                self._clear_dashboard_data()
            elif choice == '5':
                self._fix_dashboard_data_issues()
            elif choice == '0':
                break
            else:
                print("❌ 无效选择，请重试")
    
    def _fluss_architecture_generator(self):
        """通过Fluss架构生成大屏数据"""
        print("\n🏗️  Fluss架构数据生成模式")
        print("="*50)
        print("此模式在Source端生成数据，通过Fluss数仓架构传播到Sink端ADS表")
        print("确保数据流的完整性和一致性")
        print("="*50)
        ...
        try:
            ...
            print(f"\n📊 当前数据状态:")
            print(f"   Source端总记录数: {source_stats}")
    ...
    def _show_dashboard_data_status(self):
        """显示当前大屏数据状态"""
        print("\n📊 当前大屏数据状态")
        print("="*50)
            ...
            with self.get_sink_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    if total_ads_records == 0:
                        ...
                        print("💡 建议: 使用'直接操作大屏数据'模式快速生成测试数据")
                    else:
                        print("\n✅ ADS表有数据，Grafana大屏应该能正常显示")
                    ...
                    try:
                        ...
                        if rows:
                            for row in rows:
                                print(f"   📊 {row[0]}: {row[1]} {row[2]} (更新时间: {row[3]})")
                        else:
                            print("   📊 表中无数据")
                    except Exception as e:
                        print(f"   ❌ 查询详细信息失败: {e}")
                ...
                with conn.cursor() as cursor:
                    ...
                    for i in range(equipment_count):
                        ...
                        cursor.execute("""
                            INSERT INTO equipment_info (equipment_id, equipment_name, equipment_type,
                                                       location, voltage_level, capacity, manufacturer, status)
                    ...
                    for i in range(100):  # 生成100条电力数据
                        ...
                        """, (
                            ...
                            random.uniform(10, 100),    # current_a
                            random.uniform(10, 100),    # current_b
                            random.uniform(10, 100),    # current_c
                    ...
                    for i in range(20):  # 生成20条告警数据
                        ...
                        cursor.execute("""
                            INSERT INTO alert_records
                            (equipment_id, alert_type, alert_level, alert_title, alert_description, alert_time, status)
    ...
    def _generate_basic_dashboard_data_fixed(self):
        """生成基础大屏测试数据 - 修复版"""
        print("\n🚀 开始生成基础大屏测试数据 (修复版)...")
        print("此操作将直接在Sink端ADS表生成测试数据，用于快速验证大屏显示")
            ...
            with self.get_sink_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    dashboard_metrics = [
                        ...
                        ('power_factor_avg', 0.92, '', '平均功率因数', 'POWER'),
                        ('voltage_stability', 98.5, '%', '电压稳定性', 'POWER'),
                        ('equipment_online_rate', 94.8, '%', '设备在线率', 'EQUIPMENT'),
                        ('energy_efficiency', 87.3, '%', '能效指标', 'EFFICIENCY'),
                        ('load_balance_index', 0.85, '', '负载均衡指数', 'POWER'),
                        ('maintenance_alerts', 5, '个', '维护告警数', 'ALERT'),
        ...
        """, (metric_name, value, unit, desc, category, metric_time))
                    ...
                    print("="*60)
                    print("📊 数据生成汇总:")
                    print(f"   - 实时监控指标: {len(dashboard_metrics)} 条")
                    print(f"   - 设备健康度: {len(equipment_data)} 条")
                    print(f"   - 客户行为分析: {len(customer_data)} 条")
                    print("   - 告警统计: 1 条")
                    print(f"   - 电力质量: {len(quality_data)} 条")
                    print("   - 风险评估: 1 条")
                    print(f"   - 能效分析: {len(efficiency_data)} 条")
                    total_count = len(dashboard_metrics) + len(equipment_data) + len(customer_data) + 1 + len(quality_data) + 1 + len(efficiency_data)
                    print(f"   总计: {total_count} 条记录")
                    print("\n💡 现在可以刷新Grafana大屏查看数据显示效果")
                    print("\n⚠️  注意: ads_realtime_dashboard表是专门为大屏创建的，与其他Fluss架构表独立")
                    print("   如需测试完整的Fluss数据流，请使用'架构化数据生成'模式")
